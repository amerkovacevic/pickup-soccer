...
const Dashboard = () => {
  ...
  const { availableGames, joinedGames } = useGames();
      ...
      <section className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-xl">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
            ...
            <p className="mt-2 text-3xl font-semibold text-pitch-200">{groups.length}</p>
            <p className="mt-1 text-xs text-slate-500">Communities you can organize matches with.</p>
            <Link to="/groups" className="mt-4 inline-flex text-xs font-semibold text-pitch-200 hover:text-pitch-100">
              Manage groups →
            </Link>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
          ...
          {joinedGames.slice(0, 2).map((game) => (
            <article key={game.id} className="rounded-2xl border border-pitch-500/30 bg-slate-900/70 p-5 shadow-lg">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-pitch-500/20 px-3 py-1 text-xs text-pitch-200">{game.groupName}</span>
              </div>
            </article>
          ))}
          {!joinedGames.length && (
